    ...
    return (
      <div className="Settings">
        <Section text="Drawing">
          <Input ref={c => this._offsetX = c} icon={require('./icons/offset.svg')} tooltip="Offset X" initialValue={settings.render.offset.x} onChange={(v) => this._handleChange({ render: { offset: { x: v } } })} />
          <Input ref={c => this._offsetY = c} icon={require('./icons/offset_y.svg')} tooltip="Offset Y" initialValue={settings.render.offset.y} onChange={(v) => this._handleChange({ render: { offset: { y: v } } })} />
          <Input ref={c => this._zoom = c} icon={require('./icons/size.svg')} tooltip="Zoom" initialValue={settings.render.zoom} onChange={(v) => this._handleChange({ render: { zoom: v } })} />
          ...
          <Input icon={require('./icons/iteration.svg')} tooltip="Iterations" initialValue={settings.render.iterations} onChange={(v) => this._handleChange({ render: { iterations: v } })} />
          ...
          <div className="Input">
            <img data-tip="Colors" src={require("./icons/colour.svg")} style={{ marginRight: '0.5em' }}/>
          ...
          <Input icon={require('./icons/step_angle.svg')} tooltip="Angle's change" initialValue={settings.generate.angleStep} wheelChange={0.01} onChange={(v) => this._handleChange({ generate: { angleStep: v } })} />
          <Input icon={require('./icons/step_step.svg')} tooltip="Step's change" initialValue={settings.generate.stepStep} wheelChange={0.01} onChange={(v) => this._handleChange({ generate: { stepStep: v } })} />
        </Section>
      </div>
    );
  }
